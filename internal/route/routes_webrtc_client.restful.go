...
func SetupRestfulWebRTCClientRoutes(ctx context.Context, r *gin.Engine, host string, port string, logger *logrus.Logger, helloService *service.HelloService, healthService *service.HealthService, webRTCClientService *service.WebRTCClientService) {
	...
	webRTCClientController := controller.NewWebRTCClientController(ctx, logger, webRTCClientService)
	...
	webRTCGroup := r.Group("/webRTC-client")
	{
		// Get health check
		webRTCGroup.GET("/health", webRTCClientController.Health)

		// Send message to server
		webRTCGroup.GET("/send", webRTCClientController.Send)
	}
}
