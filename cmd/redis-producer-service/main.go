...
// go run ./cmd/redis-producer-service/main.go
// swag init -g cmd/redis-producer-service/main.go -o doc/openapi
// go build -o ./bin/redis-producer-service ./cmd/redis-producer-service
...
func startGinServer() {
	...
	route.SetupRestfulRedisServerRoutes(ctx, router, host, port, logger, helloService, healthService, redisServerService)
	// route.SetupGraphQLRoutes(ctx, router, host, port, logger, authService, userService, teamService, helloService, healthService, redisServerService)
...
func main() {
	...
	// Create the redis server service
	redisServerService, err = service.NewRedisServerService(ctx, logger, bootstrapServers, topics)
	if err != nil {
		logger.Fatalf("Failed to create redis server service: %v", err)
	}
	redisServerService.Initialize()
	defer redisServerService.Disconnect()
