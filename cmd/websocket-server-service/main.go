...
// go run ./cmd/websocket-server-service/main.go
// swag init -g cmd/websocket-server-service/main.go -o doc/openapi
// go build -o ./bin/websocket-server-service ./cmd/websocket-server-service
...
func startGinServer() {
	...
	route.SetupRestfulWebsocketServerRoutes(ctx, router, host, port, logger, helloService, healthService, websocketServerService)
...
func main() {
	...
	websocketServerService = service.NewWebsocketServerService(ctx, logger)
