...
func SetupRestfulWebRTCServerRoutes(ctx context.Context, r *gin.Engine, host string, port string, logger *logrus.Logger, helloService *service.HelloService, healthService *service.HealthService, webRTCServerService *service.WebRTCServerService) {
	...
	webRTCServerController := controller.NewWebRTCServerController(ctx, logger, webRTCServerService)
	...
	docGroup := r.Group("/doc")
	{
		...
		// openapiDocs.SwaggerInfowebRTC_server_service.BasePath = "/"
	...
	{
		// Get webRTC handler
		webRTCGroup.GET("/handler", webRTCServerController.WebRTCHandler)
		// webRTCGroup.GET("/echo", webRTCServerController.WebRTCEchoHandler)
		// webRTCGroup.GET("/broadcast", webRTCServerController.WebRTCBroadcastHandler)
		// webRTCGroup.GET("/event", webRTCServerController.WebRTCEventHandler)
		// webRTCGroup.GET("/message", webRTCServerController.WebRTCMessageHandler)
		// webRTCGroup.GET("/health", webRTCServerController.WebRTC HealthHandler)
	}
}
