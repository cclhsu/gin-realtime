...
func startGinServer() {
	...
	route.SetupRestfulWebsocketClientRoutes(ctx, router, host, port, logger, helloService, healthService, websocketClientService)
	// route.SetupGraphQLRoutes(ctx, router, host, port, logger, authService, userService, teamService, helloService, healthService, websocketClientService)
...
func main() {
	...
	// Create the websocket client service
	websocketClientService = service.NewWebsocketClientService(ctx, logger)
