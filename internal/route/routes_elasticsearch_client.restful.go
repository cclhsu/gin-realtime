...
func SetupRestfulElasticsearchClientRoutes(ctx context.Context, r *gin.Engine, host string, port string, logger *logrus.Logger, helloService *service.HelloService, healthService *service.HealthService, elasticsearchClientService *service.ElasticsearchClientService) {
	...
	elasticsearchClientController := controller.NewElasticsearchClientController(ctx, logger, elasticsearchClientService)
	...
	docGroup := r.Group("/doc")
	{
		...
		// openapiDocs.SwaggerInfoelasticsearch_client_service.BasePath = "/"
	...
	elasticsearchGroup := r.Group("/elasticsearch-client")
	{
		// Get health check
		elasticsearchGroup.GET("/health", elasticsearchClientController.Health)

		// Send message to server
		elasticsearchGroup.GET("/send", elasticsearchClientController.Send)
	}
}
